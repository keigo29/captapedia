...
app.post("/api/addPageData", async (req, res)=>{
    try {
        ...
        const newPageEntry = new Page(null, title, description, imageUrl, content, catalog_id);
        await newPageEntry.addPage();
    } catch (error) {
        res.status(500).json({
            error: "ページ追加の際にエラーが発生しました。"
        });
    }
});
app.post("/api/addData", async (req, res)=>{
    try {
        ...
        // データベースに登録
        const newCatalogEntry = new Catalog(null, title, description, imageUrl);
...
var __webpack_exports__ = __webpack_require__.X(0, [889], () => (__webpack_exec__(61)));
